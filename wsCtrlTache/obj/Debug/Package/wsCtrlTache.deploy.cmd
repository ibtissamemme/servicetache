...
set _DeploySetParametersFile=%RootPath%wsCtrlTache.SetParameters.xml
)
...
@rem ---------------------------------------------------------------------------------
if not exist "%RootPath%wsCtrlTache.zip" (
echo "%RootPath%wsCtrlTache.zip" does not exist. 
...
echo. "%MSDeployPath%\msdeploy.exe" -source:package='%RootPath%wsCtrlTache.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension %_MsDeployAdditionalFlags%
"%MSDeployPath%\msdeploy.exe" -source:package='%RootPath%wsCtrlTache.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension %_MsDeployAdditionalFlags%
) else (
echo. "%MSDeployPath%\msdeploy.exe" -source:package='%RootPath%wsCtrlTache.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -setParamFile:"%RootPath%wsCtrlTache.SetParameters.xml" %_MsDeployAdditionalFlags%
"%MSDeployPath%\msdeploy.exe" -source:package='%RootPath%wsCtrlTache.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -setParamFile:"%RootPath%wsCtrlTache.SetParameters.xml" %_MsDeployAdditionalFlags%
)
goto :eof
...
:usage
echo =========================================================
if not exist "%RootPath%wsCtrlTache.deploy-readme.txt" (
...
start notepad "%RootPath%wsCtrlTache.deploy-readme.txt"
)
echo =========================================================
goto :eof
...
:CheckParameterFile
echo =========================================================
if  exist "%RootPath%wsCtrlTache.SetParameters.xml" (
echo SetParameters from:
echo "%RootPath%wsCtrlTache.SetParameters.xml"
echo You can change IIS Application Name, Physical path, connectionString
echo or other deploy parameters in the above file.
) else (
echo SetParamterFiles does not exist in package location.
